"""Phase 5E deterministic verification, retry and recovery for Alfred Core.

This layer wraps the existing policy-gated executor. It does not change policy,
approval scope, tool arguments, workflow bindings or verification semantics.
Instead it adds metadata-only operation receipts, bounded read retries and
mutation replay protection. Any ambiguous mutation outcome stops for explicit
reconciliation rather than being replayed.
"""
...
MAX_READ_ATTEMPTS = 3
NON_RETRYABLE_READ_ERRORS = {"ValueError", "LookupError", "PermissionError"}
...
def _effect_class(action: str) -> str:
    ...
    return str(tool.get("risk") or "high_impact")
    ...
    with connection() as db:
        ...
        db.execute("""INSERT INTO core_reliability_operations
            ...
            ON CONFLICT(operation_key) DO UPDATE SET
              ...
              execution_id = COALESCE(excluded.execution_id, core_reliability_operations.execution_id),
              attempt_count = MAX(core_reliability_operations.attempt_count, excluded.attempt_count),
              updated_at = CURRENT_TIMESTAMP""",
            (key, request_id, plan_id, step_index, action, effect, args_hash,
             state, execution_id, count),
        )
...
def _record_receipt(*, key: str, execution_id: str, request_id: str,
    ...
    verification_state = "verified" if (verification or {}).get("ok") is True else (
        "failed" if verification is not None else "unknown"
    )
    with connection() as db:
        db.execute("""INSERT OR IGNORE INTO core_execution_receipts
            ...
            (str(uuid4()), key, execution_id, request_id, plan_id, step_index,
             action, _effect_class(action), argument_hash(arguments), attempt_number,
             outcome_state, verification_state, error_type),
        )


def _row_error_type(execution_id: str) -> str | None:
    with connection() as db:
        row = db.execute(
...
def _reconciliation_result(operation: dict) -> dict:
    execution_item = _execution(operation.get("execution_id"))
    if execution_item:
        base = _execution_result(execution_item, replayed=True)
    else:
        base = {
            ...
            "verification": None,
            "approval": None,
            "error": "Mutation outcome is ambiguous and must be reconciled before retry.",
        }
    base["state"] = "reconciliation_required"
    base["error"] = "Mutation outcome is ambiguous and must be reconciled before retry."
    return _with_reliability(base, operation, recovery="reconciliation_required", replay_protected=True)
...
async def _retry_read(operation: dict, arguments: dict) -> dict:
    ...
    if not isinstance(execution_id, str) or not execution_id:
        ...
        _upsert_operation(
            ...
            action=operation["action"], arguments=arguments, state="retry_exhausted",
        )
        refreshed = _operation(operation["operation_key"]) or operation
        return _with_reliability(result, refreshed, recovery="retry_exhausted")
    ...
    while int(current.get("attempt_count") or 0) < MAX_READ_ATTEMPTS:
        ...
        if error_type in NON_RETRYABLE_READ_ERRORS:
            _upsert_operation(
                ...
                action=current["action"], arguments=arguments, state="failed_terminal",
                execution_id=execution_id,
            )
            refreshed = _operation(current["operation_key"]) or current
            item = _execution(execution_id)
            result = _execution_result(item, replayed=True) if item else {
                "id": execution_id, "request_id": current["request_id"],
                "plan_id": current.get("plan_id"), "step_index": current.get("step_index"),
                "action": current["action"], "state": "failed", "result": None,
                "verification": None, "approval": None, "error": error_type,
            }
            return _with_reliability(result, refreshed, recovery="not_retryable")

        retried = await recovery.retry_execution(execution_id)
        attempt_number = int(current.get("attempt_count") or 0) + 1
        verification = retried.get("verification") if isinstance(retried, dict) else None
        retry_error = retried.get("error_type") if isinstance(retried, dict) else None
        _record_receipt(
            ...
            verification=verification, error_type=retry_error,
        )
        next_state = "verified" if retried.get("state") == "completed" and (verification or {}).get("ok") is True else "retryable"
        _upsert_operation(
            key=current["operation_key"], request_id=current["request_id"],
            plan_id=current.get("plan_id"), step_index=current.get("step_index"),
            action=current["action"], arguments=arguments, state=next_state,
        ...
        if next_state == "verified":
            item = _execution(execution_id)
            result = _execution_result(item, replayed=False) if item else dict(retried)
            return _with_reliability(result, current, recovery="verified")

    _upsert_operation(
        key=current["operation_key"], request_id=current["request_id"],
        plan_id=current.get("plan_id"), step_index=current.get("step_index"),
        action=current["action"], arguments=arguments, state="retry_exhausted",
        execution_id=execution_id,
    )
    current = _operation(current["operation_key"]) or current
    item = _execution(execution_id)
    result = _execution_result(item, replayed=True) if item else {
        ...
        "plan_id": current.get("plan_id"), "step_index": current.get("step_index"),
        "action": current["action"], "state": "failed", "result": None,
        "verification": None, "approval": None, "error": "Read retry budget exhausted",
    }
    result["state"] = "failed"
...
def reconcile_interrupted_operations() -> dict:
    """Classify interrupted work without replaying it."""
    initialise()
    with connection() as db:
        rows = db.execute("""SELECT id, request_id, plan_id, step_index, action, arguments
    ...
    for row in rows:
        ...
        key = operation_key(
            request_id=row["request_id"], action=row["action"], arguments=arguments,
            plan_id=row["plan_id"], step_index=row["step_index"],
        )
        state = "retryable" if _effect_class(row["action"]) == "read" else "reconciliation_required"
        existing = _operation(key)
        count = max(1, int((existing or {}).get("attempt_count") or 0))
        _upsert_operation(
            key=key, request_id=row["request_id"], plan_id=row["plan_id"],
            step_index=row["step_index"], action=row["action"], arguments=arguments,
            state=state, execution_id=row["id"], attempt_count=count,
        )
        counts[state] += 1
    return dict(counts)
...
def status() -> dict:
    ...
    with connection() as db:
        operation_rows = db.execute(
            "SELECT state, COUNT(*) AS count FROM core_reliability_operations GROUP BY state"
        ).fetchall()
        receipts = int(db.execute("SELECT COUNT(*) FROM core_execution_receipts").fetchone()[0])
    states = {str(row["state"]): int(row["count"]) for row in operation_rows}
    return {
...
def reconciliation_items(limit: int = 50) -> list[dict]:
    ...
    with connection() as db:
        rows = db.execute("""SELECT operation_key, request_id, plan_id, step_index,
                   action, effect_class, state, execution_id, attempt_count, updated_at
...
def install() -> None:
    """Wrap the existing executor and recovery path without replacing policy."""
    global _INSTALLED
    if _INSTALLED:
        return
    ...
    async def execute_tool_with_reliability(*, request_id: str, action: str, arguments: dict,
        ...
        if prior and prior["state"] == "running":
            item = _execution(prior.get("execution_id"))
            if item and item.get("state") == "completed" and (item.get("verification") or {}).get("ok") is True:
                _upsert_operation(
                ...
                refreshed = _operation(key) or prior
                return _with_reliability(
                    _execution_result(item, replayed=True), refreshed,
            ...
            if effect != "read":
                _upsert_operation(
                    key=key, request_id=request_id, plan_id=plan_id,
                    step_index=step_index, action=action, arguments=arguments,
                    state="reconciliation_required", execution_id=prior.get("execution_id"),
                )
                return _reconciliation_result(_operation(key) or prior)

        _upsert_operation(
            key=key, request_id=request_id, plan_id=plan_id, step_index=step_index,
            action=action, arguments=arguments, state="running",
        )
        ...
        current = _operation(key) or {
            "operation_key": key, "request_id": request_id, "plan_id": plan_id,
            "step_index": step_index, "action": action, "attempt_count": 0,
        }
        ...
        if item is not None and not result.get("replayed"):
            ...
            _record_receipt(
                ...
                verification=verification, error_type=_row_error_type(execution_id),
            )
            _upsert_operation(
                key=key, request_id=request_id, plan_id=plan_id,
                step_index=step_index, action=action, arguments=arguments,
                state="running", execution_id=execution_id, attempt_count=attempt_number,
            )
            current = _operation(key) or current
        elif item is not None and result.get("replayed"):
            _upsert_operation(
                key=key, request_id=request_id, plan_id=plan_id,
                step_index=step_index, action=action, arguments=arguments,
        ...
        if state == "completed" and (verification or {}).get("ok") is True:
            ...
            current = _operation(key) or current
            return _with_reliability(
                result, current, recovery="verified", replay_protected=effect != "read",
            )
        ...
        if state in {"failed", "interrupted"} and effect != "read":
            ...
            record_audit(
                ...
                {"execution_id": execution_id, "action": action, "operation_key": key},
                request_id,
            )
            return _reconciliation_result(_operation(key) or current)

        terminal = "approval_required" if state == "approval_required" else (
            "denied" if state == "denied" else str(state or "failed_terminal")
        )
        _upsert_operation(
            key=key, request_id=request_id, plan_id=plan_id,
            step_index=step_index, action=action, arguments=arguments,
        ...
        return _with_reliability(result, _operation(key) or current, recovery=terminal)
    ...
    async def run_goal_with_reliability(goal_id: str, *, trigger: str = "owner") -> dict:
        ...
        if goal and goal.get("state") == "blocked":
            ...
            step = next((item for item in goal.get("steps", []) if item["id"] == current_id), None)
            if step and _effect_class(str(step.get("action"))) == "read":
                with connection() as db:
                    row = db.execute("""SELECT id, request_id, plan_id, step_index, action, arguments, state
                        FROM core_executions WHERE plan_id = ? AND step_index = ?
                ...
                if row and row["state"] in {"failed", "interrupted"}:
                    ...
                    key = operation_key(
                        request_id=row["request_id"], action=row["action"], arguments=args,
                        plan_id=row["plan_id"], step_index=row["step_index"],
                    )
                    _upsert_operation(
                        key=key, request_id=row["request_id"], plan_id=row["plan_id"],
                        step_index=row["step_index"], action=row["action"], arguments=args,
                        state="retryable", execution_id=row["id"], attempt_count=1,
                    )
                    recovered = await _retry_read(_operation(key) or {}, args)
                    if recovered.get("state") == "completed":
                        with connection() as db:
                            db.execute(
                                "UPDATE plans SET state = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                                (goal["plan_id"],),
                            )
