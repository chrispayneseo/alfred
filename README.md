...
- `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` come from a Google Cloud Console OAuth client (Web application type). In the Cloud Console you need both the **Calendar API** and the **Gmail API** enabled for the project (Library → search each → Enable), and the OAuth consent screen needs the `calendar.events`, `gmail.readonly`, `gmail.compose`, and `userinfo.email` scopes, with every Google account you plan to connect added as a test user (the project stays in "Testing" mode, so each account needs to be added explicitly — see [Multiple Google accounts](#multiple-google-accounts)). `calendar.events` was `calendar.readonly` before Chat gained the ability to create events — accounts connected under the old scope need reconnecting once (Settings → Google accounts) before event creation works; everything else keeps working unaffected in the meantime.
...
Read + write on events only (`calendar.events` — not calendar settings/ACLs), one or more accounts (see [Multiple Google accounts](#multiple-google-accounts)), OAuth 2.0 with `access_type: offline` + `prompt: consent` so a refresh token is always issued. `server/google/calendar.ts` is the shared, standalone service module: `listEvents(env, range)` for one account, plus `listEventsAllAccounts(accounts, range)` (and `getTodayEventsAllAccounts`/`getTomorrowEventsAllAccounts` convenience wrappers) that merge every connected account's events, sorted by start time and tagged with `accountEmail`; `createEvent(env, input)` creates a single event. A missing or revoked token (or one that predates the `calendar.events` upgrade and lacks write access) surfaces as a typed `GoogleNotConnectedError` / `GoogleReconnectRequiredError` rather than a generic failure, so both the Today screen and Chat can show an honest "reconnect" state instead of crashing or guessing — and with multiple accounts, one account needing reconnection doesn't hide the other's still-working events (see [Multiple Google accounts](#multiple-google-accounts)).

**Chat can create events, but only with your explicit confirmation.** Chat has no general tool-calling loop — this is a single narrow capability. When a request looks like it's asking for a calendar event, the model is instructed to *propose* one (title/date/time, defaulting to the `cpayneer@gmail.com` account unless the other is named) rather than create it: it emits a `[[CALENDAR_EVENT_PROPOSAL]]` JSON block on a line by itself, which `server/llm/chat.ts` parses out of the reply (never shown to the user as raw text) into a structured `eventProposal` on the API response. The frontend renders that as a card with explicit **Add to calendar** / **Cancel** buttons — nothing is written to your calendar until you tap Add, which calls `POST /api/calendar/create-event`. `server/llm/chat.ts` also always injects the real current date (in `Europe/London`, `DEFAULT_TIME_ZONE` from `calendar.ts`) into the model's context, so "tomorrow"/"next Friday" resolve correctly instead of guessing from training data.
...
**Scope review.** `calendar.events`, `gmail.readonly`, `gmail.compose`, and `userinfo.email` — each still the narrowest scope available for what its feature does (`calendar.events` covers reading and creating events but not calendar settings/ACLs or other calendars; no `gmail.modify`, no `gmail.send`; `userinfo.email` grants nothing beyond reading the connected account's own address). `calendar.events` replaced `calendar.readonly` once Chat gained the ability to create events (see [Google Calendar](#google-calendar)) — a real, deliberate widening, not scope creep: it's still bounded to events specifically, and every write still requires the user's explicit confirmation in the UI before it happens (`server/llm/chat.ts` only ever *proposes*; `POST /api/calendar/create-event` is the sole place anything is actually written, and nothing calls it except that confirmed UI action). Nothing broader than necessary is requested, on any connected account.
